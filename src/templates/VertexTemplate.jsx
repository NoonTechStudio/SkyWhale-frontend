          ...
          <div className="relative z-10">
            ...
            <div className="bg-white border border-teal-100 rounded-2xl p-5 shadow-sm mb-6">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <p className="text-gray-800 font-bold text-xl">{safeClientData.ownerName}</p>
                  <p className="text-teal-600 text-base">{safeClientData.ownerTitle}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="px-6 pb-6 min-h-[400px]">
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-6 duration-700">
              ...
              <div className="bg-white border border-teal-100 rounded-2xl p-6 shadow-sm hover:shadow-md transition-all">
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    <h4 className="text-xs font-bold text-teal-600 uppercase tracking-wider mb-2">Location</h4>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-white border border-teal-100 rounded-2xl p-5 shadow-sm">
                  ...
                  <a href={`tel:${safeClientData.phone}`} className="text-gray-800 font-semibold text-base hover:text-teal-600 transition break-all">
                ...
                <div className="bg-white border border-teal-100 rounded-2xl p-5 shadow-sm">
                  ...
                  <a href={`mailto:${safeClientData.email}`} className="text-gray-800 font-semibold text-sm break-all hover:text-teal-600 transition">
              ...
              <div className="bg-white border border-teal-100 rounded-2xl p-6 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center gap-2">
                    ...
                    <h4 className="text-base font-bold text-gray-800">Business Hours</h4>
                  </div>
                ...
                <div className="space-y-3">
                  {safeClientData.workingHours.map((day, index) => (
                    <div key={index} className="flex items-center justify-between text-sm">
                      <span className={`font-medium ${day.closed ? 'text-gray-400' : 'text-gray-700'}`}>
                        {day.day}
                      </span>
                      {day.closed ? (
                        <span className="text-amber-500 text-sm font-semibold">Closed</span>
                      ) : (
                        <span className="text-gray-600 font-mono">{day.open} - {day.close}</span>
                      )}
                    </div>
                  ))}
                </div>
              ...
              {Object.values(safeClientData.social).some(link => link && link !== '#') && (
                <div className="bg-white border border-teal-100 rounded-2xl p-6 shadow-sm">
                  <h4 className="text-xs font-bold text-teal-600 uppercase tracking-wider mb-4">Connect With Us</h4>
          ...
          {activeSection === "services" && (
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-6 duration-700">
              <h3 className="text-xl font-bold text-gray-800 mb-4">
                {safeClientData.businessType === 'service' ? 'Our Premium Services' : 'Our Products'}
              </h3>
              {safeClientData.items.length > 0 ? safeClientData.items.map((item, idx) => (
                ...
                >
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      <h4 className="text-gray-800 font-bold text-lg mb-2">{item.name}</h4>
                      {item.description && (
                  ...
                  {safeClientData.payment.upiId && (
                    ...
                    >
                      <div>
                        ...
                        <p className="text-gray-800 font-mono text-base">{safeClientData.payment.upiId}</p>
                      </div>
                      <div className="w-10 h-10 bg-white rounded-lg flex items-center justify-center text-teal-600">
                        {copiedUpi ? <CheckCircle2 size={20} /> : <Copy size={20} />}
                      </div>
                    </button>
                  )}
                  ...
                  {safeClientData.payment.bankDetails?.accountNumber && (
                    <div className="mt-4 pt-4 border-t border-teal-100">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        <h4 className="text-sm font-bold text-gray-800">Bank Transfer</h4>
                      </div>
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          <span className="text-gray-500">Account:</span>
                          <span className="text-gray-800 font-mono">{safeClientData.payment.bankDetails.accountNumber}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-500">IFSC:</span>
                          <span className="text-gray-800 font-mono">{safeClientData.payment.bankDetails.ifscCode}</span>
                        </div>
                      </div>
                    </div>
                  )}
          ...
          {activeSection === "gallery" && (
            <div className="space-y-4 animate-in fade-in zoom-in-95 duration-700">
              <h3 className="text-xl font-bold text-gray-800 mb-4">Our Gallery</h3>
