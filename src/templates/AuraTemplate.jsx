...
const AuraTemplate = ({ clientData = sampleClient }) => {
  ...
  const safeClientData = {
    ...
    whatsapp: clientData?.contact?.whatsapp || clientData?.whatsapp || clientData?.contact?.phone || "",
    email: clientData?.contact?.email || clientData?.email || "",
    address: clientData?.contact?.address || clientData?.address || "Address not provided",
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-white to-slate-50 font-sans">
      <div className="max-w-md mx-auto bg-white shadow-xl min-h-screen relative overflow-hidden">
        ...
        <div className="px-6 mb-4 mt-4">
          <div className="grid grid-cols-4 gap-2">
            ...
            >
              ...
              <span className="text-[8px] font-bold text-blue-700 uppercase tracking-wider">Call</span>
            </a>
            <a
            ...
            >
              ...
              <span className="text-[8px] font-bold text-green-700 uppercase tracking-wider">Chat</span>
            </a>
            <a
            ...
            >
              ...
              <span className="text-[8px] font-bold text-purple-700 uppercase tracking-wider">Mail</span>
            </a>
            {safeClientData.googleMapsUrl && safeClientData.googleMapsUrl !== '#' ? (
              ...
              >
                ...
                <span className="text-[8px] font-bold text-red-700 uppercase tracking-wider">Maps</span>
              </a>
            ) : (
              <button disabled className="flex flex-col items-center gap-1 p-3 bg-gray-100 rounded-xl cursor-not-allowed">
                <MapPin size={18} className="text-gray-400" />
                <span className="text-[8px] font-bold text-gray-400 uppercase tracking-wider">Maps</span>
              </button>
            )}
          </div>
        </div>
          ...
          <div className="bg-slate-50 rounded-2xl p-1.5 border border-slate-200">
            <div className="grid grid-cols-4 gap-1">
              {["about", "services", "gallery", "payment"].map((tab) => (
                ...
                >
                  {tab === "services" ? (safeClientData.businessType === 'service' ? 'Services' : 'Products') : tab}
                </button>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-bottom-6 duration-700">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="bg-white border border-slate-200 rounded-2xl p-4">
                  ...
                  <a href={`tel:${safeClientData.phone}`} className="text-sm text-slate-800 font-medium break-all">
                ...
                <div className="bg-white border border-slate-200 rounded-2xl p-4">
                  ...
                  <a href={`mailto:${safeClientData.email}`} className="text-sm text-slate-800 font-medium break-all">
                  ...
                  {safeClientData.workingHours.map((day, idx) => (
                    <div key={idx} className="flex justify-between text-xs">
                      ...
                      {day.closed ? (
                        ...
                        <span className="text-slate-700">{day.open} - {day.close}</span>
                      )}
                    </div>
                  ))}
                </div>
              ...
              {Object.values(safeClientData.social).some(link => link && link !== '#') && (
                <div className="bg-white border border-slate-200 rounded-2xl p-5">
                  <h4 className="text-sm font-bold text-slate-800 mb-3">Connect With Us</h4>
              ...
              {safeClientData.items.length > 0 ? (
                safeClientData.items.map((item, idx) => (
                  <div key={idx} className="bg-white border border-slate-200 rounded-2xl p-4">
                    ...
                    {item.price && <span className="text-sm font-bold text-green-600">₹{item.price}</span>}
                  </div>
                ))
              ) : (
                <p className="text-center text-slate-500 py-8">No services listed</p>
              )}
            </div>
          )}
          ...
          {activeSection === "gallery" && (
            <div>
              ...
              {safeClientData.gallery.length > 0 ? (
                ...
                <p className="text-center text-slate-500 py-8">No gallery images</p>
              )}
            </div>
          )}
              ...
              <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-2xl p-6 text-white">
                ...
                {safeClientData.payment.qrCode ? (
                  <div className="bg-white p-4 rounded-xl mb-4">
                    <img src={getImageUrl(safeClientData.payment.qrCode)} alt="QR Code" 
                ...
                {safeClientData.payment.upiId && (
                  <button onClick={() => copyToClipboard(safeClientData.payment.upiId)}
                          className="w-full bg-white/10 border border-white/20 rounded-xl p-3 flex justify-between items-center">
                    <span className="text-sm">{safeClientData.payment.upiId}</span>
                    {copiedUpi ? <CheckCircle2 size={18} className="text-green-400" /> : <Copy size={18} />}
                  </button>
                )}
              </div>
            </div>
        ...
        <div className="bg-gradient-to-br from-slate-900 to-slate-800 text-white text-center py-6 px-6">
          ...
          <p className="text-xs text-slate-600 mt-3">© 2026 • Aura Digital Card</p>
        </div>
      </div>
    </div>
  );
